...
function preload() {
  // Load images
  bgImage = loadImage('game_background.png');  // Replace with actual path
  introBgImage = loadImage('intro_background.png');  // Replace with actual path
...
function draw() {
  if (!gameStarted) {
    ...
    // Game is running
    background(50);  // Background color (default, will be overwritten by image below)

    image(bgImage, 0, 0, width, height);  // Display game background
...
function showStartMenu() {
  image(introBgImage, 0, 0, width, height);  // Display intro background
  textSize(50);
  fill(255, 10, 10);
  textAlign(CENTER, CENTER);
...
class Zombie {
  ...
  display() {
    if (this.type === 'fast') {
      image(zombieImageFast, this.x, this.y, this.size, this.size);  // Fast zombie
    } else if (this.type === 'special') {
      image(zombieImageSpecial, this.x, this.y, this.size, this.size);  // Special zombie
    } else {
      image(zombieImageNormal, this.x, this.y, this.size, this.size);  // Normal zombie
    }
  }
  ...
  reset() {
    ...
    this.size = this.type === 'special' ? 100 : (this.type === 'small' ? 75 : 90); // be careful code (brain of zombie)
